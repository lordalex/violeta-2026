...
class VioletaSupabaseUser extends BaseAuthUser {
  VioletaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> violetaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = VioletaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
